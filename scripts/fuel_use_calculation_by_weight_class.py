...
for b in range(len(Weights)):
    ...
    # Initialise a final array
    results_names = ['origin', 'destination', 'length_m', 'regular_L', 'hybrid_L', 'regular_$', 'hybrid_$', 'geometry']
    combined_gdf = pd.DataFrame(columns=results_names)

    # run through calculations for every .gpkg file OD pairing
    for filename in os.listdir('../results/routes_fixed_elevations'):
        print(os.path.join('../results/routes_fixed_elevations', filename))
        if filename.endswith(".gpkg"):
            ...
            '''
            #read routes
            routes = gpd.read_file('../results/routes_fixed_elevations/'+filename)
            ...
            # append all data into on dataframe
            combined_gdf = combined_gdf.append(results_gdf)
            # print(combined_gdf)
    ...
    name = str(Weights[b]) + '_Tonne_Ouput.xlsx'
    # print to excel
    combined_gdf.to_excel(name, engine='xlsxwriter')
    print('Weight = ', Weights[b], ' Tonnes - complete')